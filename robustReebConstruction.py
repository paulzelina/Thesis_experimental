...
def constructRobustReeb(streamlines, eps, alpha, delta, threshold):
    print("Constructing...")
    """
    ...
    # set adjacency threshold for clustering
    threshold = threshold

    # resample streamlines to a predetermined number of points
    feature = ResampleFeature(nb_points=40)
    metric = AveragePointwiseEuclideanMetric(feature=feature)

    # perform streamline clustering
    ...
    while (process_flag):
        ...
        for cluster_pres in clusters_pres:
            if cluster_pres not in clusters_prev:
                ...
                for cluster_traj in cluster_pres:
                    if stream_list[cluster_traj][0] < len(streamlines[cluster_traj]) and assign_cluster[cluster_traj][
                        stream_list[cluster_traj][0]] == -1:
                        assign_cluster[cluster_traj][stream_list[cluster_traj][0]] = assign_cluster[cluster_traj][
                            stream_list[cluster_traj][0] - 1]
        ...
        if stream_i not in del_s_id:
            ...
            if len(unique_cluster) == 1:
                ...
                if not cluster_edge.get(unique_cluster[0]):
                    ...
                    R[node_id][node_id + 1]['weight'] = count_trajectories[unique_cluster[0]] / sum(
                        cluster_map.values())
            ...
            for uc in range(len(unique_cluster) - 1):
                if not cluster_edge.get(unique_cluster[uc]):
                    R.add_edge(node_id, node_id + 1)
                    R[node_id][node_id + 1]['weight'] = count_trajectories[unique_cluster[uc]] / sum(
                        cluster_map.values())
                    cluster_edge[unique_cluster[uc]] = [node_id, node_id + 1]
                    node_id += 2
                if not cluster_edge.get(unique_cluster[uc + 1]):
                    R.add_edge(node_id, node_id + 1)
                    R[node_id][node_id + 1]['weight'] = count_trajectories[unique_cluster[uc + 1]] / sum(
                        cluster_map.values())
    ...
    for node_key in node_loc.keys():
        ...
        for nk in node_loc[node_key]:
            ...
            n_y += nk[1]
            n_z += nk[2]
        node_loc_final[node_key] = [n_x / len(node_loc[node_key]), n_y / len(node_loc[node_key]),
    ...
    for stream_i in range(len(streamlines)):
        if stream_i not in del_s_id:
            ...
            for uc in range(len(unique_cluster) - 1):
                ...
                dist1 = distance(node_loc_final[cluster_edge[unique_cluster[uc]][1]],
                                 node_loc_final[cluster_edge[unique_cluster[uc + 1]][0]])
                dist2 = distance(node_loc_final[cluster_edge[unique_cluster[uc]][1]],
        ...
        if len(cluster) > 1:
            for c in cluster:
                ...
                if node_id in node_loc_final:
                    node_loc_final[node_id] = [node_loc_final[node_id][0] / 2 + node_loc_final[c][0] / 2,
